...
import { getDynamoDBDocumentClient, getPayload, flattenKeys } from './helpers.mjs'
...
export const handler = async (event) => {
    ...
    for (let entry of body.entry) {
        for (let change of entry.changes) {
            ...
            let baseItem = {
                AccountId: entry.id,
                MessagingProduct: entry.messaging_product,
            ...
            const statusItems = (change.value.statuses || []).map(status => {
                return formatPutRequest(baseItem, status, 'status')
            })
...
function formatPutRequest(baseItem, notification, notificationType) {
    let customerId
    let payload = getPayload(notification)
    ...
    return {
        PutRequest: {
            Item: {
                ...
                SortKey: `message#whatsapp#${notification.timestamp}#${Math.random().toString(16).slice(2)}`,
